...
void InitOpponents(tRace_info* pRace_info) {
    ...
    for (i = 0, opponent_number = 0; i < gProgram_state.AI_vehicles.number_of_opponents; i++, opponent_number++) {
        ...
        // Assign random AI profile and log it
        int profile_count = 2; // Update if you add more profiles
        gProgram_state.AI_vehicles.opponents[i].ai_profile = (eAIProfile)(rand() % profile_count);
        dr_dprintf("%s: Using AI profile %d, car_ID %x",
            gProgram_state.AI_vehicles.opponents[i].car_spec->driver_name,
            gProgram_state.AI_vehicles.opponents[i].ai_profile,
